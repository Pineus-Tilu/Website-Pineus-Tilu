<nav class="fixed top-0 left-0 z-50 w-full bg-white shadow-sm">
    ...
    <ul class="hidden md:flex items-center space-x-6 lg:space-x-8 text-[18px] lg:text-[20px] font-medium text-green-800 font-typewriter">
      ...
      <li class="relative group">
        <button type="button" class="flex items-center hover:underline underline-offset-4 focus:outline-none">
          Fasilitas
          <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path d="M19 9l-7 7-7-7"></path>
          </svg>
        </button>
        <ul class="dropdown-menu absolute left-0 z-50 mt-2 w-52 bg-white hidden group-hover:block border rounded shadow-md text-green-800 text-[18px] font-typewriter">
      ...
      @auth
        <li class="relative group">
          <button type="button" class="flex items-center px-4 py-2 text-white bg-[#006C43] rounded-md hover:bg-green-700 focus:outline-none">
            {{ Auth::user()->name }}
            <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              <path d="M19 9l-7 7-7-7"></path>
            </svg>
          </button>
          <ul class="dropdown-menu absolute right-0 z-50 hidden group-hover:block mt-2 w-48 bg-white border rounded shadow-md text-green-800 text-[16px] font-typewriter">
    ...
    <!-- Tombol Menu Mobile -->
    <button id="mobile-menu-toggle" class="text-green-800 focus:outline-none md:hidden">
  ...
  <div id="mobile-menu" class="hidden px-4 pt-2 pb-4 space-y-2 text-green-800 bg-white border-t md:hidden font-typewriter">
    ...
    <div x-data="{ open: false }">
      ...
      <div x-show="open" x-transition class="mt-1 ml-4 space-y-1">
        @foreach($areas as $area)
          <a
            href="/fasilitas/{{ strtolower(str_replace([' ', '(', ')'], ['-', '', ''], $area->name)) }}"
