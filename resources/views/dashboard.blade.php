...
@section('content')
    <div class="max-w-full overflow-hidden" x-data="{
        ...
        <div x-show="showModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
            ...
            class="fixed inset-0 z-[99999] flex items-center justify-center bg-black bg-opacity-95" style="display: none;">
            ...
            <div class="relative flex items-center justify-center w-full h-full p-4 overflow-hidden">
                <img :src="modalImg" :alt="modalTitle"
                    ...
                    @mouseup="isDragging = false" @mouseleave="isDragging = false" draggable="false" />
            </div>
        ...
        <!-- Modal Denah -->
        <div x-show="showDenah" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click.self="closeDenah()"
        ...
        <div x-show="showMapModal" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" @click.self="closeMapModal()" @keydown.escape.window="closeMapModal()"
            class="fixed inset-0 z-[99999] bg-black bg-opacity-90 flex items-center justify-center p-4"
            style="display: none;">

            <!-- Modal Content -->
            <div
                class="relative w-full max-w-7xl h-full max-h-[90vh] bg-white rounded-xl md:rounded-2xl overflow-hidden shadow-2xl">
                <!-- Modal Header -->
                <div
                    class="flex items-center justify-between p-4 text-white md:p-6 bg-gradient-to-r from-green-600 to-blue-600">
                ...
                <div class="relative h-full">
                    <iframe src="https://maps.google.com/maps?q=Pineus%20Tilu&t=&z=15&ie=UTF8&iwloc=&output=embed"
                        class="w-full h-full border-0" title="Pineus Tilu Interactive Map" loading="lazy"></iframe>
