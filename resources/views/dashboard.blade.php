...
@section('content')
    ...
    <section class="bg-[#006C43] py-16 px-4 md:px-8 text-white" data-aos="fade-up" data-aos-duration="1000"
        ...
        <div class="max-w-6xl mx-auto text-center">
            ...
            <div class="relative">
                <div class="w-full swiper mySwiper font-typewriter">
                    <div class="swiper-wrapper">
                        @foreach ($slides as $slide)
                            <div class="flex flex-col overflow-hidden bg-white rounded-lg shadow-lg cursor-pointer swiper-slide"
                                @click="showModal = true; modalImg = '{{ asset('images/' . $slide['img']) }}'; modalTitle = '{{ $slide['title'] }}'"
                                data-aos="zoom-in-up" data-aos-delay="{{ 100 * $loop->index }}">
        ...
        <!-- Modal Preview Gambar (pindahkan ke sini, di luar .max-w-6xl dan .relative) -->
        <div x-show="showModal" x-transition @click.self="showModal = false"
            class="fixed inset-0 z-[99999] bg-black bg-opacity-80 flex items-center justify-center"
            style="display: none;">
            <button @click="showModal = false"
                class="absolute top-4 right-4 z-[100000] p-2 text-white bg-black bg-opacity-50 rounded-full hover:bg-opacity-75">
                ✕
            </button>
            <div class="relative z-[10000] w-full h-full flex items-center justify-center overflow-y-auto">
    ...
    <section class="py-12 bg-white" data-aos="fade-up" data-aos-duration="1000">
        <div class="container max-w-full px-4 mx-auto">
            <h2 class="mb-6 text-3xl font-bold text-center text-green-800 jp-brush" data-aos="fade-right">Awal mula Pineus
                Tilu</h2>
                ...
                <div x-show="open" x-transition @click.self="open = false"
                    ...
                    <div class="fixed z-[100000] left-1/2 bottom-8 -translate-x-1/2">
                        <div class="flex px-3 py-2 space-x-2 rounded shadow bg-white/80">
                            ...
                            <button @click.stop="zoomOut" class="px-3 py-1 text-black bg-white rounded shadow">−</button>
                            <button @click.stop="resetZoom" class="px-3 py-1 text-black bg-white rounded shadow">⟳</button>
                        </div>
                    </div>
                </div>
            </template>
    ...
    <section class="py-12 bg-white" data-aos="fade-up" data-aos-duration="1000">
        <div class="container px-4 mx-auto">
            <h2 class="mb-6 text-2xl font-bold text-center text-green-800 jp-brush font-cursive" data-aos="fade-down">Our
                Location</h2>
...
<nav class="fixed top-0 left-0 z-50 w-full bg-white shadow-sm">
  <div class="flex items-center justify-between w-full px-4 py-2 sm:px-4 lg:px-10">
    <!-- Logo -->
