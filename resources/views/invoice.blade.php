...
<body>
    @php
        ...
        $checkIn = $booking->bookingDetail ? \Carbon\Carbon::parse($booking->bookingDetail->check_in) : \Carbon\Carbon::now();
        $checkOut = $booking->bookingDetail ? \Carbon\Carbon::parse($booking->bookingDetail->check_out) : \Carbon\Carbon::now()->addDay();
        $nights = $checkIn->diffInDays($checkOut);
        if ($nights == 0) $nights = 1;
        
        $basePrice = $booking->bookingDetail ? ($booking->bookingDetail->total_price - ($booking->bookingDetail->extra_charge ?? 0)) : 0;
        ...
        // Invoice number
        $invoiceNumber = $booking->payment->order_id ?? $booking->invoice_number ?? 'INV-' . str_pad($booking->id, 6, '0', STR_PAD_LEFT);
    @endphp
    ...
    <div class="header-section clearfix">
        ...
        <div class="header-right invoice-info">
            <p style="margin: 2px 0; font-size: 8px;"><strong>Date :</strong> {{ $booking->created_at->format('d F Y') }}</p>
            <p style="margin: 2px 0; font-size: 8px;"><strong>Invoice to :</strong> {{ $booking->bookingDetail->nama ?? $booking->user->name ?? 'Guest' }}</p>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td>Harga Dasar - {{ $booking->unit->area->name ?? 'Area' }} {{ $booking->unit->unit_name ?? 'Deck' }} ({{ $priceType }}, {{ $nights }} malam)</td>
                <td class="text-center">{{ $nights }}</td>
    ...
    <table>
        <thead>
            <tr>
                <th colspan="4" class="text-right" style="font-size: 9px; font-weight: bold; background-color: #2c5530; color: white;">TOTAL</th>
                <th class="text-right" style="font-size: 9px; font-weight: bold; background-color: #2c5530; color: white;">Rp {{ number_format($subTotal, 0, ',', '.') }}</th>
            </tr>
        </thead>
    </table>
    ...
    <div class="payment-info">
        <h4 style="margin: 0 0 8px 0; font-size: 10px; color: #2c5530; border-bottom: 1px solid #2c5530; padding-bottom: 3px; font-weight: bold; text-transform: uppercase;">Informasi Pembayaran</h4>
        <table class="no-border" style="width: 100%; font-size: 8px;">
            <tr>
                <td style="width: 60%; vertical-align: top; padding: 5px;">
                    <table class="no-border" style="width: 100%;">
                        ...
                        <tr>
                            ...
                            <td class="no-border" style="color: #2c5530; font-weight: bold;">Rp {{ number_format($booking->payment->gross_amount ?? $subTotal, 0, ',', '.') }}</td>
                        </tr>
                        <tr>
                ...
                <td style="width: 40%; text-align: right; vertical-align: top; padding: 5px;">
                    <p style="margin: 0; font-size: 8px; font-weight: bold; color: #2c5530;">Bandung, {{ \Carbon\Carbon::now()->format('d F Y') }}</p>
    ...
    <div class="contact-info">
        <div class="info-box">
            ...
            <p style="margin: 3px 0 0 0; font-size: 8px;">Bank BCA | a.n. CV. Pineus Tilu Trans | No Rek: <strong>7710183018</strong></p>
        </div>
    </div>
    ...
    <div class="header-section clearfix">
        <div class="header-left">
            <img src="{{ public_path('images/pineus_tilu_logo.png') }}" alt="Pineus Tilu Riverside" style="max-height: 50px; width: auto;">
        </div>
        <div class="header-right invoice-info">
    ...
    <div class="rules-section">
        <h3 style="color: #2c5530; border-bottom: 2px solid #2c5530; padding-bottom: 10px;">PERATURAN SELAMA BERADA DI PINEUS TILU</h3>
    ...
    <div class="rules-section">
        ...
        <div class="cancellation-policy">
            <h4 style="text-align: center; margin: 15px 0; color: #2c5530; font-size: 10px;">KETENTUAN PEMBATALAN BERLAKU PER TENDA</h4>
            <p style="text-align: center; font-style: italic; margin-bottom: 15px; color: #666; font-size: 7px;">CANCELLATION TERMS APPLY PER TENT</p>
            
            <div class="clearfix">
                <div class="two-column">
                    <div class="policy-box" style="border-left: 3px solid #dc3545;">
                        <h5 style="color: #dc3545; text-align: center; margin-bottom: 8px; font-size: 8px; text-transform: uppercase;">BAHASA INDONESIA</h5>
                        <p style="margin: 3px 0; font-weight: bold; color: #dc3545; font-size: 7px;">Di luar H-7 tanggal check-in = Potongan 25%</p>
                        <p style="margin: 3px 0; font-weight: bold; color: #dc3545; font-size: 7px;">Terhitung H-7 hingga H-4 = Potongan 50%</p>
                        <p style="margin: 3px 0; font-weight: bold; color: #dc3545; font-size: 7px;">Terhitung H-3 hingga H-2 = Potongan 75%</p>
                        <p style="margin: 3px 0; font-weight: bold; color: #dc3545; font-size: 7px;">Terhitung H-1 hingga H = Potongan 100%</p>
                    </div>
                </div>
                <div class="two-column">
                    <div class="policy-box" style="border-left: 3px solid #007bff;">
                        <h5 style="color: #007bff; text-align: center; margin-bottom: 8px; font-size: 8px; text-transform: uppercase;">ENGLISH VERSION</h5>
                        <p style="margin: 3px 0; color: #007bff; font-weight: bold; font-size: 7px;">Before D-7 check-in date = Cut 25%</p>
                        <p style="margin: 3px 0; color: #007bff; font-weight: bold; font-size: 7px;">Counting D-7 to D-4 = Cut 50%</p>
                        <p style="margin: 3px 0; color: #007bff; font-weight: bold; font-size: 7px;">Counting D-3 to D-2 = Cut 75%</p>
                        <p style="margin: 3px 0; color: #007bff; font-weight: bold; font-size: 7px;">Counting D-1 to D day = Cut 100%</p>
                    </div>
                </div>
            </div>

            <div style="margin: 15px 0;">
                <h4 style="text-align: center; color: #2c5530; background-color: #f8f9fa; padding: 5px; font-size: 9px; text-transform: uppercase;">PENGAJUAN PEMBATALAN</h4>
                <p style="text-align: center; font-style: italic; margin-bottom: 10px; color: #666; font-size: 7px;">CANCELLATION REQUEST</p>
                
                <div class="clearfix">
                    <div style="width: 65%; float: left; background-color: #f8f9fa; padding: 8px; border-left: 3px solid #28a745;">
                        <h5 style="color: #28a745; margin-bottom: 5px; font-size: 7px; font-weight: bold;">PEMBATALAN DAPAT DILAKUKAN DENGAN MENGHUBUNGI ADMIN WHATSAPP PINEUS TILU</h5>
                        <p style="font-style: italic; color: #666; margin: 2px 0; font-size: 6px;">Cancellation can be made by contacting Pineus Tilu WhatsApp admin</p>
                        <p style="margin: 5px 0; font-weight: bold; font-size: 7px;">WhatsApp: +62 812-3456-7890</p>
                    </div>
                    <div style="width: 30%; float: right; background-color: #fff3cd; padding: 8px; border-left: 3px solid #ffc107;">
                        <h5 style="color: #856404; margin-bottom: 5px; font-size: 7px; font-weight: bold;">JAM OPERASIONAL:</h5>
    ...
    <div style="margin-top: 40px; text-align: center; padding-top: 20px; border-top: 1px solid #ddd;">
        ...
        <p style="margin: 5px 0; font-size: 10px; color: #666;">Jl. Raya Pangalengan, Rahong, Pangalengan, Bandung, Jawa Barat</p>
